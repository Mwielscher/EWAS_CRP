...
>>* we used this [script](EWAS_CRP/OverRepresentation_Analysis/bin_SD_for_permutation.R) to bin the 450k probes according their standard errors observed in the participating cohorts, those were stored in this [file](EWAS_CRP/OverRepresentation_Analysis/450K_SD_bins_for_permutation.RData)    
>>* we counted the number of probes in each bin for the significant loci also given in this [script](EWAS_CRP/OverRepresentation_Analysis/bin_SD_for_permutation.R)  
...
>>* First we used this [script]() to create a [wget script]() for data download  
>>* For this data we first had to [add the actual chromosomal postion]() to the files and then we combined those files into [one file per chromatin state]()  
>>* Then we run the actual [script]() that performes the perumtation tests  
>>* The perumutation test script produces one file per chromatin state - those were combined with this helper [script]()  
